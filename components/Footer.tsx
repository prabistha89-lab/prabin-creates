    ...
    <footer className="site-footer">
      <div className="container footer-grid">
        <div className="footer-intro">
          <Link href="/" className="brand brand-light"><span className="brand-mark">DP</span><span>Designer <b>Prabin</b></span></Link>
          <p>Thoughtful print and digital design for businesses that value clarity, craft and consistent presentation.</p>
          <div className="social-row">
            {siteConfig.socials.map((social) => <a key={social.label} href={social.href} target="_blank" rel="noreferrer" aria-label={social.label}>{social.label.slice(0, 2)}</a>)}
          </div>
        </div>
        ...
        <div><h2>Start a project</h2><a href={`mailto:${siteConfig.email}`}><Icon name="mail" size={17} />{siteConfig.email}</a><a href={`tel:${siteConfig.phoneHref}`}><Icon name="phone" size={17} />{siteConfig.phone}</a><p className="footer-location"><Icon name="map-pin" size={17} />{siteConfig.location}</p></div>
      </div>
