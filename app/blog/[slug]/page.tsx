...
export default async function ArticlePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <>
    <article>
      ...
      <div className={`article-feature-art blog-art blog-art-${post.art}`}><span>{post.category}</span><b>{post.title.split(" ").slice(0, 5).join(" ")}</b><i /></div>
      <div className="container article-layout">
        ...
        <div className="article-body">
          ...
          {toc.map((section) => <section id={section.id} key={section.id}>
